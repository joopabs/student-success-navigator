...
- Student Success Navigator Dash app (optional Step 8): `python -m ssn app` · guide [`docs/DEPLOYMENT.md`](docs/DEPLOYMENT.md) · six pages (Overview, Support Queue, Student Review, New Record Scoring, Equity Dashboard, Model Card) · loads the saved pipeline, never retrains · acknowledgement-gated local action log, read back into the queue as handled/remaining status so an adviser can work the list without contacting anyone twice · 35 app tests under `tests/app/`
