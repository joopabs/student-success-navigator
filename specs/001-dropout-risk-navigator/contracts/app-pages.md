...
| `/queue` | Support Queue | scored demo cohort, `capacity.k` | Top-K table: `record_id`, score (2 dp), band, top-2 neutral factors; notice if K > cohort; K selector limited to `k_sensitivity` values | Open record; acknowledge review (modal) → record action / dismiss / override with optional reason | Outcomes, sensitive attributes, any name/email |
