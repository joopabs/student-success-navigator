...
- **Decision**: One `configs/base.yaml` holding `seed`, paths, split ratio, `capacity_k`,
  `k_sensitivity`, band rule, `min_group_size`, `illustrative_assumptions` block; per-model YAML
...
- **Decision**: Primary treatment is `class_weight="balanced"` (LR, RF, HGB) plus
  capacity-aware thresholding. Secondary experiment: SMOTE via `imblearn.pipeline.Pipeline`
  inside CV for LR and RF, reported in `cv_comparison.csv`. Selection prefers class weighting
  unless SMOTE improves PR-AUC by more than one CV standard deviation, because weighting is
...
- **Decision**: K is defined at cohort level (the demo/test cohort). In CV, each validation fold
  ...
  Report Recall@K and Precision@K for `capacity_k` and for each value in `k_sensitivity`.
- **Illustrative default (assumption, not a fact)**: `capacity_k: 50`, `k_sensitivity: [25, 50,
  100]`. This is a placeholder for a weekly adviser outreach capacity; every surface that shows it
  carries the label "illustrative". PV-10 records the final choice.
...
- **Decision**: `data/raw`, `data/processed`, `data/demo`, `data/evaluation`, `data/local` are
  Git-ignored and regenerated by `make data`. `models/final_pipeline.joblib` is committed only if
  under 10 MB; otherwise Git-ignored with `make fit-final` as the regeneration step and its
  sha256 recorded in `models/manifest.json` (always committed). Figures and tables under
...
- **Decision**: `shap.TreeExplainer` for RF/HGB, `shap.LinearExplainer` for logistic regression
  ...
  Fallback: `sklearn.inspection.permutation_importance`. PDP/ICE via
